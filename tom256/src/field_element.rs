pub trait FieldElement {
    const MODULUS: U256;
    /// auxiliary variable for multiplication
    ...
    const fn barrett_reduce(lo: U256, hi: U256) -> Self {
        ...
        const fn q3_times_n_keep_five(q3: &[u64; 5]) -> [u64; 5] {
            ...
            let (w0, carry) = mac(0, q3[0], limbs[0].0, 0);
            let (w1, carry) = mac(0, q3[0], limbs[1].0, carry);
            let (w2, carry) = mac(0, q3[0], limbs[2].0, carry);
            let (w3, carry) = mac(0, q3[0], limbs[3].0, carry);
            let (w4, _) = mac(0, q3[0], 0, carry);

            let (w1, carry) = mac(w1, q3[1], limbs[0].0, 0);
            let (w2, carry) = mac(w2, q3[1], limbs[1].0, carry);
            let (w3, carry) = mac(w3, q3[1], limbs[2].0, carry);
            let (w4, _) = mac(w4, q3[1], limbs[3].0, carry);

            let (w2, carry) = mac(w2, q3[2], limbs[0].0, 0);
            let (w3, carry) = mac(w3, q3[2], limbs[1].0, carry);
            let (w4, _) = mac(w4, q3[2], limbs[2].0, carry);

            let (w3, carry) = mac(w3, q3[3], limbs[0].0, 0);
            let (w4, _) = mac(w4, q3[3], limbs[1].0, carry);

            let (w4, _) = mac(w4, q3[4], limbs[0].0, 0);
        ...
        const fn subtract_n_if_necessary(r0: u64, r1: u64, r2: u64, r3: u64, r4: u64) -> [u64; 5] {
            let limbs = Self::MODULUS.limbs();
            let (w0, borrow) = sbb(r0, limbs[0].0, 0);
            let (w1, borrow) = sbb(r1, limbs[1].0, borrow);
            let (w2, borrow) = sbb(r2, limbs[2].0, borrow);
            let (w3, borrow) = sbb(r3, limbs[3].0, borrow);
            let (w4, borrow) = sbb(r4, 0, borrow);

            // If underflow occurred on the final limb, borrow = 0xfff...fff, otherwise
            // borrow = 0x000...000. Thus, we use it as a mask to conditionally add the
            // modulus.
            let (w0, carry) = adc(w0, limbs[0].0 & borrow, 0);
            let (w1, carry) = adc(w1, limbs[1].0 & borrow, carry);
            let (w2, carry) = adc(w2, limbs[2].0 & borrow, carry);
            let (w3, carry) = adc(w3, limbs[3].0 & borrow, carry);
