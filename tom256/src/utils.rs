...
fn get_random_u256<C: CryptoRng + RngCore>(rng: &mut C) -> U256 {
    let mut bytes = [0_u8; 32];
    rng.fill_bytes(&mut bytes);
    U256::from_be_bytes(bytes)
}

pub fn get_random_modular<T: Modular>() -> T {
    let mut rng = ChaChaRng::from_entropy();
    loop {
        let random_number = get_random_u256(&mut rng);
...
mod test {
    ...
    fn get_random_small_modular<T: Modular>(mod_byte_number: u8) -> T {
        let mut rng = ChaChaRng::from_entropy();
        loop {
            let random_number_bytes = get_random_u256(&mut rng).to_be_bytes();
    ...
    fn test_rand() {
        ...
        for i in 0..1000000 {
            let rand_num = get_random_small_modular::<TestModular>(1);
